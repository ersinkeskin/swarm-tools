...
void linkMex(double *m1, double *m2, double *linkIL, double *posx, double *posy, double *ort, double eta_ts_rad, double *psi,unsigned int seed,long mLen,long iterLim, double b)
{
    long i,idx1,idx2;
    double linkdistx;
    double linkdisty;
...
//     double b = -250*1;    
    ...
    for(iter=0;iter<iterLim;iter++) {
        ...
        for (i=0; i<mLen; i++) {
            ...
            temp = (b_ts*(dist-distLink1[i]) + k*(dist-linkIL[i]))/dist;
            distLink1[i] = dist;
        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    posy = mxGetPr(prhs[4]);
    ort = mxGetPr(prhs[5]);
    eta_ts_rad = mxGetScalar(prhs[6]);
    seed = (unsigned int)mxGetScalar(prhs[7]);
    mLen = (long)mxGetScalar(prhs[8]);
    iterLim = (long)mxGetScalar(prhs[9]);
    ...
    linkMex(m1,m2,linkIL,posx,posy,ort,eta_ts_rad,psi,seed,mLen,iterLim,b);
}
